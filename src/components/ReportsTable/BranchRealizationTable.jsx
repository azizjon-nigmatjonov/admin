...
const BranchRealizationTable = () => {
  ...
  const fetchData = () => {
    setLoader(true);
    ReportService.branch_Realization_getList()
      .then((res) => setData(res?.branch_realization_data))
        ...
        {!loader && data.length ? (
          <>
            {data?.map((row, rowIndex) => (
              <CTableRow key={row.branch_id}>
                <CTableCell>{rowIndex + 1}</CTableCell>
                <CTableCell>{row.branch_name}</CTableCell>
...
export default BranchRealizationTable;
