...
const MatrixRolePage = () => {
  ...
  const getRecordPermissions = (slug = "record_permission") => {
  ...
  const handleReadPermission = (data, appId) => {
    if ('read' in data) {
      const newData = {
      ...
      roleService.updateReadPermission({data: newData}).then(() => {
        getRecordPermissions();
      })
    }
  }
  ...
  const { mutate: updateAppPermission } = useMutation(
    ({ data, appId }) => {
      handleReadPermission(data, appId)
      roleService.updateAppPermission({ data }, appId)
    },
    {
      onSuccess: () => {
        setParentPopupKey("");
